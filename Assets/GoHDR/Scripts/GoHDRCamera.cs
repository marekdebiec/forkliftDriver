...
//			gohdrManager.SetLightWeight(1f);
	...
	public void Start(){
		...
		//camScaledHeight = Mathf.FloorToInt( renderCamera.pixelHeight * camScaledWidth / renderCamera.pixelWidth );
		lumCamScaledHeight = (int)(GetComponent<Camera>().pixelRect.height * .125f);
		//lumCamScaledHeight = (int)(camera.pixelRect.height * .5f);
		lumCamScaledWidth = Mathf.FloorToInt( lumCamScaledHeight * GetComponent<Camera>().aspect );
//		Debug.Log("lum cam: " + lumCamScaledWidth + " x " + lumCamScaledHeight);
		
		if (bloomOn) {
			dsCamScaledHeight = (int)(GetComponent<Camera>().pixelRect.height * .5f);
			dsCamScaledWidth = Mathf.FloorToInt( dsCamScaledHeight * GetComponent<Camera>().aspect );
...
//		_tex.Apply();
	...
	void OnGUI()
    {
		...
		if (bloomOn) {
       		Graphics.DrawTexture(GetComponent<Camera>().pixelRect, dummyTexture, bloomMaterial);
		}
    }
...
//			Debug.Log("CreateRenderObjects");
			...
			Transform luminosityCamGOTransform = transform.Find("GoHDR Luminosity Camera");
			Transform dsCamGOTransform = transform.Find("GoHDR DS Camera");
			...
			if (bloomOn) {
				if (null != dsCamGOTransform) {
					...
					dsCamGO.GetComponent<Camera>().CopyFrom( GetComponent<Camera>() );
			...
			luminosityCamera = luminosityCamGO.GetComponent<Camera>();
            luminosityCamera.enabled = true;
...
//			Debug.Log("luminosityCamera.pixelRect: " + luminosityCamera.pixelRect.ToString() ); 
			...
			if (bloomOn) {
				downsampledCamera = dsCamGO.GetComponent<Camera>();
				downsampledCamera.enabled = true;
